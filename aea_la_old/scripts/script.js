...
class GraphVisualization {
    ...
    async loadInfoMarkdown() {
        try {
            const response = await fetch('markdown/info.md');
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
            const markdown = await response.text();
            // Basic markdown to HTML conversion (can be replaced with a library like 'marked' if needed)
            const html = this.convertMarkdownToHtml(markdown);
            document.getElementById('markdown-container').innerHTML = html;
        } catch (error) {
            console.error('Error loading info.md:', error);
            document.getElementById('markdown-container').innerHTML = '<p class="text-red-500">Fehler beim Laden der Informationen.</p>';
        }
    }

    convertMarkdownToHtml(markdown) {
        // Improved conversion for basic markdown. For more complex markdown, a library is recommended.
        let html = markdown
            // Horizontal Rule
            .replace(/^(---|\*\\*\\*|___)$/gim, '<hr>')
            // Headings
