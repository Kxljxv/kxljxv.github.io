...
class GraphVisualization {
    ...
    async loadData() {
        try {
            document.getElementById('loading-text').textContent = 'Lade bdk_data.json...';
            const response = await fetch('./data_gathering/bdk_data.json');
            if (!response.ok) {
            ...
            document.getElementById('loading').innerHTML = `
                      <div class="max-w-md mx-auto bg-red-900/20 border border-red-500 rounded-lg p-6">
                          ...
                          <ul class="list-disc list-inside text-gray-400 space-y-1">
                              ...
                              <li>Die Datei data_gathering/bdk_data.json existiert</li>
    ...
    async simulatePhysics() {
        ...
        const res = await fetch('data_gathering/simulated/nodes.json');
    ...
    setupEventListeners() {
        ...
        const kvBtn = document.getElementById('kv-create-btn');
        if (kvBtn) {
            kvBtn.addEventListener('click', () => this.openKvGroupModal());
        }
        const kvSearch = document.getElementById('kv-search');
        if (kvSearch) {
            kvSearch.addEventListener('input', (e) => this.renderKvList(e.target.value));
        }
        const kvConfirm = document.getElementById('kv-confirm');
        if (kvConfirm) {
            kvConfirm.addEventListener('click', () => this.confirmKvGroupCreation());
        }
    }
    ...
    buildKvIndex() {
        console.log("Baue KV-Index auf...");
        this.kvMap = new Map();
        const addKv = (kv, node) => {
            if (!kv) return;
            const key = kv.trim();
            if (!key) return;
            const list = this.kvMap.get(key) || [];
            list.push(node);
            this.kvMap.set(key, list);
        };

        // Regex für verschiedene Formate: "(KV Berlin-Mitte)" oder "Berlin-Mitte"
        const kvRegex = /(?:\(KV\s+([^)]+)\)|KV\s+([^)]+))/i;

        this.allNodes.forEach(node => {
            if (node.type !== 'supporter') return;
            
            let kv = null;
            // Zuerst sublabel prüfen
            if (node.sublabel && typeof node.sublabel === 'string') {
                const m = node.sublabel.match(kvRegex);
                if (m) {
                    kv = (m[1] || m[2]).trim();
                } else if (node.sublabel.trim().length > 0) {
                    kv = node.sublabel.trim();
                }
            }
            
            // Falls nichts im sublabel gefunden, label prüfen
            if (!kv && typeof node.label === 'string') {
                const m = node.label.match(kvRegex);
                if (m) {
                    kv = (m[1] || m[2]).trim();
                }
            }
        ...
        this.kvIndex = Array.from(this.kvMap.keys()).sort((a, b) => a.localeCompare(b));
        console.log(`KV-Index fertig: ${this.kvIndex.length} KVs gefunden.`);
    }

    openKvGroupModal() {
        console.log("Versuche KV-Modal zu öffnen...");
        if (!this.kvIndex || this.kvIndex.length === 0) {
            console.warn("Keine KV-Informationen gefunden.");
            alert('Keine KV-Informationen gefunden.');
            return;
        }
        
        this.selectedKv = null;
        const modalEl = document.getElementById('kv-group-modal');
        if (!modalEl) {
            console.error("Modal-Element 'kv-group-modal' nicht gefunden!");
            return;
        }

        // Flowbite Modal initialisieren oder abrufen
        let modal = FlowbiteInstances.getInstance('Modal', 'kv-group-modal');
        if (!modal) {
    ...
    renderKvList(query) {
        ...
        const items = (q ? this.kvIndex.filter(k => k.toLowerCase().includes(q)) : this.kvIndex).slice(0, 500);
        
        if (items.length === 0) {
            listEl.innerHTML = '<p class="text-[hsl(var(--text-400))] text-sm p-2">Keine KV gefunden</p>';
            return;
        }

        listEl.innerHTML = items.map(kv => {
            const count = this.kvMap.get(kv)?.length || 0;
            const safeKv = kv.replace(/'/g, "\\'").replace(/"/g, "&quot;");
            return `
                <label class="flex items-center justify-between p-2 rounded cursor-pointer text-sm mb-1 bright-container hover:bg-[hsl(var(--bg-200)/0.5)] transition-colors">
                    <div class="flex items-center gap-3">
                        <input type="radio" name="kv-choice" value="${safeKv}" 
                               ${this.selectedKv === kv ? 'checked' : ''} 
                               class="w-4 h-4 text-[hsl(var(--accent-main-100))] bg-[hsl(var(--bg-200))] border-[hsl(var(--border-300)/0.3)] focus:ring-[hsl(var(--accent-main-100)/0.3)]"
                               onchange="window.graph.selectKvChoice('${safeKv}')">
                        <span class="text-[hsl(var(--text-000))] font-medium">${kv}</span>
                    </div>
                    <span class="text-[hsl(var(--text-400))] text-xs bg-[hsl(var(--bg-200))] px-2 py-1 rounded-full">${count} Pers.</span>
                </label>
            `;
        }).join('');
    }
    ...
    selectKvChoice(kv) {
        console.log(`KV ausgewählt: ${kv}`);
        this.selectedKv = kv;
    }

    confirmKvGroupCreation() {
        const kv = this.selectedKv;
        if (!kv) {
            alert('Bitte einen KV auswählen.');
            return;
        }
        
        const nodes = this.kvMap.get(kv) || [];
        if (nodes.length === 0) {
            alert('Keine Unterstützer für den ausgewählten KV gefunden.');
            return;
        }
        
        const name = `KV ${kv}`;
        // Standardfarbe oder aktuelle Auswahl verwenden
        const color = this.selectedColor || '#7f6df2';
        
        // Neue Gruppe erstellen
        const groupId = Date.now();
        const group = { id: groupId, name, color, nodes: [] };
        this.groups.push(group);
        
        // Alle Nodes des KV zur Gruppe hinzufügen
        nodes.forEach(n => this.addNodeToGroup(n.id, groupId));
        
        console.log(`Gruppe '${name}' mit ${nodes.length} Nodes erstellt.`);
        ...
        // Modal schließen
        const modalEl = document.getElementById('kv-group-modal');
        const modal = FlowbiteInstances.getInstance('Modal', 'kv-group-modal');
        if (modal) {
            modal.hide();
        } else {
            // Fallback falls Instanz nicht gefunden
